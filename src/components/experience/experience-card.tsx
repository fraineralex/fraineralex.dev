    ...
    <li className={`mb-12 ${id !== 0 ? 'animate-card' : undefined}`}>
      <div className='group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50'>
        ...
        <Image
          ...
          className={`z-10 rounded transitio hidden md:inline-block md:col-span-2 mt-2 mx-auto group-hover:scale-110 ${
            imageName === 'vbs.avif' ? 'w-16' : 'w-20'
          }`}
        ...
        <aside className='z-10 sm:col-span-6'>
          <header className='font-medium leading-snug text-shark-200'>
            <h5
              className='text-white mb-1 text-sm block sm:hidden'
              aria-hidden='true'
            >
              {startDate} — {endDate}
              {/*  · {contract} */}
